...
// 檢查廠商統編是否重複
// $sql = "SELECT COUNT(IFNULL(uniform_numbers, 0)) FROM `drug_agents` WHERE uniform_numbers = '" . $uniform_numbers . "'";
// $RS = $conn->query($sql);
// list($unnum) = mysqli_fetch_row($RS);
...
// if (intval($unnum) == 0) {
	...
	if ($result = $conn->query($sql)) {
		...
		echo json_encode(array('errorMsg' => '發生資料庫錯誤: 無法存檔!!'));
	}
	
// } else {
//	echo json_encode(array('errorMsg' => '發生錯誤: [廠商統編重複]--無法存檔!!'));
// }
